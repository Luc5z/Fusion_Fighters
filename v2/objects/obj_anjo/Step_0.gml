...
if(playerinst != noone)
{
    ...
    if (object_index == obj_anjo)
    {
        // Reproduz o som do worm se não estiver sendo reproduzido
        if (!audio_is_playing(snd_obj_anjo))
        {
            audio_play_sound(snd_obj_anjo, 1, false);
        }
    }
    ...
    if (distance_to_object(playerinst) <= distancia_visao)
    {
        ...
        show_debug_message("obj_anjo avistou o jogador!");
        if (!audio_is_playing(snd_obj_anjo))
        {
    ...
    {
        show_debug_message("obj_anjo não está vendo o jogador.");
    }
