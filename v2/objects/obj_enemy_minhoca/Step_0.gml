...
//Minhoca inico
    ...
    if(playerinst != noone)
    {
        ...
        // Verifica se este é o inimigo minhoca
        if (sprite_index == obj_enemy_minhoca)
        {
            // Reproduz o som do worm se não estiver sendo reproduzido
            if (!audio_is_playing(snd_worm))
            {
                audio_play_sound(snd_worm, 1, false);
            }
        }
        ...
        var distancia_visao = 300; // Defina a distância de visão da minhoca
        if (distance_to_object(playerinst) <= distancia_visao)
        {
            // Minhoca avistou o jogador, reproduzir som de alerta
            show_debug_message("Minhoca avistou o jogador!");
            if (!audio_is_playing(snd_worm))
            {
        ...
        {
            show_debug_message("Minhoca não está vendo o jogador.");
        }
